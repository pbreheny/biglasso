...
double get_elem_bm(XPtr<BigMatrix> xpMat, double center_, double scale_, int i,
                   int j) {
...
// double crossprod_bm_raw(XPtr<BigMatrix> xpMat, double *y, int *row_idx, int
// n, int j) {
//   double res = 0.0;
//   MatrixAccessor<double> xAcc(*xpMat);
//   double *xCol = xAcc[j];
...
double crossprod_bm(XPtr<BigMatrix> xpMat, double *y_, int *row_idx_,
...
double crossprod_bm_no_std(XPtr<BigMatrix> xpMat, double *y_, int n_row,
                           int j) {
  MatrixAccessor<double> xAcc(
      *xpMat); // Initialize MatrixAccessor with the file-backed matrix
  double *xCol = xAcc[j];
  double sum = 0.0;
...
// cumulative difference (to use in residual calculation; see
// ncvreg::rawfit_gaussian lines 63 - 73 for reference)
// TODO: revisit this and make gaussian_simple setup easier to read...
// double cumdiff_bm(XPtr<BigMatrix> xpMat, NumericVector &a_, NumericVector
// &y_,
//                    int n, int p){
...
double crossprod_bm_Xj_Xk(XPtr<BigMatrix> xMat, int *row_idx,
                          NumericVector &center, NumericVector &scale, int n,
                          int j, int k) {
...
double crossprod_resid(XPtr<BigMatrix> xpMat, double *y_, double sumY_,
                       int *row_idx_, double center_, double scale_, int n_row,
                       int j) {
  MatrixAccessor<double> xAcc(*xpMat);
  double *xCol = xAcc[j];
...
// crossprod_resid - given specific rows of *standardized* X: separate
// computation
//  double crossprod_resid_no_std(XPtr<BigMatrix> xpMat, double *y_, double
//  sumY_,
//                          int n_row, int j) {
//
//    MatrixAccessor<double> xAcc(*xpMat);
...
void update_resid_eta(double *r, double *eta, XPtr<BigMatrix> xpMat,
                      double shift, int *row_idx_, double center_,
                      double scale_, int n, int j) {
...
double wcrossprod_resid(XPtr<BigMatrix> xpMat, double *y, double sumYW_,
                        int *row_idx_, double center_, double scale_, double *w,
                        int n_row, int j) {
  MatrixAccessor<double> xAcc(*xpMat);
  double *xCol = xAcc[j];
...
double wsqsum_bm(XPtr<BigMatrix> xpMat, double *w, int *row_idx_,
                 double center_, double scale_, int n_row, int j) {
  MatrixAccessor<double> xAcc(*xpMat);
  double *xCol = xAcc[j];
  ...
  val = (sum_wx_sq - 2 * center_ * sum_wx + pow(center_, 2) * sum_w) /
        pow(scale_, 2);
  return val;
}

// standardize & get residual
void standardize_and_get_residual(
    ...
    XPtr<BigMatrix> xMat, double *y, int *row_idx, double alpha, int n, int p,
    double *m) {
  MatrixAccessor<double> xAcc(*xMat);
  double *xCol;
  double sum_xy, sum_y;
...
//     if (fabs(zj) > zmax) {
//       zmax = fabs(zj);
//       *xmax_ptr = j; // xmax_ptr is the index in the raw xMat, not index in
//       col_idx!
//     }
//     z.push_back(zj);
//   }
...
static int check_kkt_set(int *test1, int val1, int *test2, int val2,
                         int *discard, int *set1, int *set2, vector<double> &z,
                         XPtr<BigMatrix> xpMat, int *row_idx,
                         vector<int> &col_idx, NumericVector &center,
                         NumericVector &scale, double *a, double lambda,
                         double sumResid, double alpha, double *r, double *m,
                         int n, int p) {
  MatrixAccessor<double> xAcc(*xpMat);
  double *xCol, sum, l1, l2;
  int j, jj, violations = 0;
#pragma omp parallel for private(j, sum, l1, l2) reduction(+ : violations)     \
    schedule(static)
  for (j = 0; j < p; j++) {
...
int check_inactive_set(int *e1, vector<double> &z, XPtr<BigMatrix> xpMat,
  ...
  return check_kkt_set(e1, 0, nullptr, 0, nullptr, e1, nullptr, z, xpMat,
                       row_idx, col_idx, center, scale, a, lambda, sumResid,
                       alpha, r, m, n, p);
}
...
int check_safe_set(int *ever_active, int *discard_beta, vector<double> &z,
                   ...
                   double lambda, double sumResid, double alpha, double *r,
                   double *m, int n, int p) {
  return check_kkt_set(ever_active, 0, nullptr, 0, discard_beta, ever_active,
                       nullptr, z, xpMat, row_idx, col_idx, center, scale, a,
                       lambda, sumResid, alpha, r, m, n, p);
}

// check KKT conditions over features in (the safe set - the strong set)
int check_rest_safe_set(int *ever_active, int *strong_set, int *discard_beta,
                        vector<double> &z, XPtr<BigMatrix> xpMat, int *row_idx,
                        vector<int> &col_idx, NumericVector &center,
                        NumericVector &scale, double *a, double lambda,
                        double sumResid, double alpha, double *r, double *m,
                        int n, int p) {
  return check_kkt_set(strong_set, 0, nullptr, 0, discard_beta, ever_active,
                       strong_set, z, xpMat, row_idx, col_idx, center, scale, a,
                       lambda, sumResid, alpha, r, m, n, p);
}
...
int check_strong_set(int *e1, int *e2, vector<double> &z, XPtr<BigMatrix> xpMat,
                     ...
                     NumericVector &scale, double *a, double lambda,
                     double sumResid, double alpha, double *r, double *m, int n,
                     int p) {
  return check_kkt_set(e1, 0, e2, 1, nullptr, e1, nullptr, z, xpMat, row_idx,
                       col_idx, center, scale, a, lambda, sumResid, alpha, r, m,
                       n, p);
}
...
int check_rest_set(int *e1, int *e2, vector<double> &z, XPtr<BigMatrix> xpMat,
                   ...
                   double sumResid, double alpha, double *r, double *m, int n,
                   int p) {
  return check_kkt_set(e2, 0, nullptr, 0, nullptr, e1, e2, z, xpMat, row_idx,
                       col_idx, center, scale, a, lambda, sumResid, alpha, r, m,
                       n, p);
}

// update z[j] for features which are rejected at previous lambda but not
// rejected at current one.
void update_zj(vector<double> &z, int *bedpp_reject, int *bedpp_reject_old,
               XPtr<BigMatrix> xpMat, int *row_idx, vector<int> &col_idx,
               NumericVector &center, NumericVector &scale, double sumResid,
...
// [[Rcpp::export]]
RcppExport SEXP get_eta(SEXP xP, SEXP row_idx_, SEXP beta, SEXP idx_p,
                        SEXP idx_l) {
  BEGIN_RCPP
  ...
  for (int k = 0; k < nnz; k++) {
    for (int i = 0; i < n; i++) {
      // double x = (xAcc[index_p[k]][row_idx[i]] - center[index_p[k]]) /
      // scale[index_p[k]];
