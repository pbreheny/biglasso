...
void standardize_and_get_residual_multiresp(
    ...
    XPtr<BigMatrix> xMat, NumericMatrix &Y, int *row_idx, double alpha, int n,
    int p, int m, double *mp, vector<double> *XtY = nullptr) {
...
void crossprod_resid_multiresp(double *xTR, XPtr<BigMatrix> xMat, double *R,
                               double *sumResid, int *row_idx, double center,
                               double scale, int n, int m, int j) {
  MatrixAccessor<double> xAcc(*xMat);
  double *xCol = xAcc[j];
  double xi;
  int i, k;
...
// Update beta
void lasso(arma::field<arma::sp_mat> &beta, double *xTR, double z, double l1,
           double l2, int j, int l, int m) {
  if (z <= l1) {
    for (int k = 0; k < m; k++) {
      beta.at(k).at(j, l) = 0;
    }
...
// utilities.cpp -- see the file-level comment at the top of this file.
void update_resid_multiresp(XPtr<BigMatrix> xpMat, double *R, double *sumResid,
                            double *shift, int *row_idx, double center,
                            double scale, int n, int m, int j) {
...
void bedpp_init(XPtr<BigMatrix> xMat, double *R, double *sumResid,
                vector<double> &XtY, double *lhs1, double *lhs2, double *lhs3,
                vector<double> &z, int xmax_idx, double lambda_max,
                int *row_idx, vector<int> &col_idx, NumericVector &center,
                NumericVector &scale, double alpha, int n, int p, int m) {
  double xjtx; // xjtx_max
  // compute x_maxtY
  double *xTR = R_Calloc(m, double);
  int j, jj, k;
  crossprod_resid_multiresp(xTR, xMat, R, sumResid, row_idx, center[xmax_idx],
                            scale[xmax_idx], n, m, xmax_idx);
...
void edpp_screen(int *discard_beta, int n, int p, int m, double lambda_prev,
                 double rhs2, double *lhs1, double *lhs2, double *lhs3,
                 double c, double d, double *mp, double alpha,
                 vector<int> &col_idx, bool EDPP) {
  int j;
  ...
  for (j = 0; j < p; j++) {
    if (EDPP) {
      if (pow(c * (1 - d), 2) * lhs1[j] + pow(1 + c * d, 2) * lhs2[j] +
              2 * c * (1 + c * d) * (1 - d) * lhs3[j] <
          pow(rhs, 2))
        discard_beta[j] = 1;
      else
        discard_beta[j] = 0;
    } else {
      if (pow(1 + c, 2) * lhs1[j] + pow(c, 2) * lhs2[j] +
              2 * c * (1 + c) * lhs3[j] <
          pow(rhs, 2))
        discard_beta[j] = 1;
      else
...
void edpp_update(XPtr<BigMatrix> xpMat, double *R, double *sumResid,
                 double *lhs2, double *lhs3, vector<double> &XtY, int *row_idx,
  ...
  int i, k, j, jj;
  double sum2, sum3;
#pragma omp parallel for schedule(static) private(i, k, j, jj, xCol, xTR,      \
                                                      sum2, sum3)
  for (j = 0; j < p; j++) {
    jj = col_idx[j];
    xCol = xAcc[jj];
...
// top of this file).
int check_strong_set_multiresp(int *e1, int *e2, vector<double> &z,
                               XPtr<BigMatrix> xpMat, int *row_idx,
                               vector<int> &col_idx, NumericVector &center,
...
#pragma omp parallel for private(i, k, j, jj, xTR, l1, l2, sum)                \
    reduction(+ : violations) schedule(static)
  for (j = 0; j < p; j++) {
    if (e1[j] == 0 && e2[j] == 1) {
      jj = col_idx[j];
      xCol = xAcc[jj];
...
// top of this file.
int check_rest_set_multiresp(int *e1, int *e2, vector<double> &z,
                             XPtr<BigMatrix> xpMat, int *row_idx,
                             vector<int> &col_idx, NumericVector &center,
                             NumericVector &scale, double *a, double lambda,
                             double *sumResid, double alpha, double *R,
  ...
  double *xCol, *xTR, l1, l2, sum;
  int i, k, j, jj, violations = 0;
#pragma omp parallel for private(i, k, j, xTR, l1, l2, sum)                    \
    reduction(+ : violations) schedule(static)
  for (j = 0; j < p; j++) {
    if (e2[j] == 0) {
      jj = col_idx[j];
...
int check_rest_safe_set_multiresp(int *e1, int *e2, int *discard_beta,
                                  vector<double> &z, XPtr<BigMatrix> xpMat,
                                  int *row_idx, vector<int> &col_idx,
                                  NumericVector &center, NumericVector &scale,
                                  double *a, double lambda, double *sumResid,
                                  double alpha, double *R, double *mp, int n,
                                  int p, int m) {
...
static void cdfit_mgaussian_lambda_core(
    arma::field<arma::sp_mat> &beta, double *a, double *R, double *sumResid,
    vector<double> &z, int *ever_active, int *strong_set, int *discard_beta,
    XPtr<BigMatrix> xMat, int *row_idx, vector<int> &col_idx,
    NumericVector &center, NumericVector &scale, double *mp, double alpha,
    double lambda_l, double thresh, int max_iter, int n, int p, int m, int l,
    IntegerVector &iter, NumericVector &loss, double *xTR, double *shift) {
  ...
  while (iter[l] < max_iter) {
    while (iter[l] < max_iter) {
        ...
        for (j = 0; j < p; j++) {
          if (ever_active[j]) {
            ...
            if (update != 0) {
              ...
              update_resid_multiresp(xMat, R, sumResid, shift, row_idx,
                                     center[jj], scale[jj], n, m,
                                     jj); // update R and sumResid
              for (k = 0; k < m; k++)
                a[j * m + k] = beta.at(k).at(j, l);
            }
          }
        }
      ...
      violations = check_strong_set_multiresp(
          ever_active, strong_set, z, xMat, row_idx, col_idx, center, scale, a,
          lambda_l, sumResid, alpha, R, mp, n, p, m);
      if (violations == 0)
        break;
    }

    // Scan for violations in rest set (or rest-safe set, if screening with
    // EDPP/BEDPP)
    if (discard_beta == nullptr) {
      violations = check_rest_set_multiresp(
          ever_active, strong_set, z, xMat, row_idx, col_idx, center, scale, a,
          lambda_l, sumResid, alpha, R, mp, n, p, m);
    } else {
      violations = check_rest_safe_set_multiresp(
          ever_active, strong_set, discard_beta, z, xMat, row_idx, col_idx,
          center, scale, a, lambda_l, sumResid, alpha, R, mp, n, p, m);
    }
    if (violations == 0) {
      loss[l] = 0;
      for (i = 0; i < n; i++) {
        for (k = 0; k < m; k++)
          loss[l] += pow(R[i * m + k], 2);
      }
      break;
    }
  }
...
RcppExport SEXP cdfit_mgaussian_ada(SEXP X_, SEXP y_, SEXP row_idx_,
                                    ...
                                    SEXP dfmax_, SEXP ncore_, SEXP safe_thresh_,
  ...
  // standardize: get center, scale; get p_keep_ptr, col_idx; get z, XtY,
  // lambda_max, xmax_idx;
  standardize_and_get_residual_multiresp(center, scale, p_keep_ptr, col_idx, z,
                                         lambda_max_ptr, xmax_ptr, xMat, Y,
                                         row_idx, alpha, n, p, m, mp, &XtY);
    ...
    if (l != lstart) {
      ...
      for (j = 0; j < p; j++) {
        if (discard_beta[j] == 1) {
          ...
          if (z[j] > (cutoff0 * alpha * mp[col_idx[j]]))
            e2[j] = 1;
          else
            e2[j] = 0;
        }
      ...
      if (gain - n_safe_reject[l - 1] * (l - l_prev) > update_thresh * p &&
          l != L - 1 && safe) { // Update EDPP if not discarding enough
        if (verbose) {
          // output time
          char buff[100];
          time_t now = time(0);
        ...
        // Reapply EDPP
        edpp_screen(discard_beta, n, p, m, lambda[l_prev], rhs2, lhs1, lhs2,
                    lhs3, c, d, mp, alpha, col_idx, EDPP);
      ...
      bedpp_init(xMat, R, sumResid, XtY, lhs1, lhs2, lhs3, z, xmax_idx,
                 lambda_max, row_idx, col_idx, center, scale, alpha, n, p, m);
      ...
      c = (lambda_max - lambda[l]) / 2 / lambda[l];
      edpp_screen(discard_beta, n, p, m, lambda_max, rhs2, lhs1, lhs2, lhs3, c,
                  0, mp, alpha, col_idx, EDPP);
      n_safe_reject[l] = sum(discard_beta, p);
      gain = n_safe_reject[l];
    }
    n_reject[l] = p - sum(e2, p);

    cdfit_mgaussian_lambda_core(beta, a, R, sumResid, z, e1, e2, discard_beta,
                                xMat, row_idx, col_idx, center, scale, mp,
                                alpha, lambda[l], thresh, max_iter, n, p, m, l,
                                iter, loss, xTR, shift);
  }
...
RcppExport SEXP cdfit_mgaussian_ssr(SEXP X_, SEXP y_, SEXP row_idx_,
  ...
  for (l = lstart; l < L; l++) {
    ...
    cdfit_mgaussian_lambda_core(beta, a, R, sumResid, z, e1, e2, nullptr, xMat,
                                row_idx, col_idx, center, scale, mp, alpha,
                                lambda[l], thresh, max_iter, n, p, m, l, iter,
                                loss, xTR, shift);
  }
