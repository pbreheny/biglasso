...
// dual function
double dual_bin(vector<double> &theta, double lambda, double lambda_max,
                int n) {
  double res = 0.0;
...
void slores_init(vector<double> &theta_lam, double *g_theta_lam_ptr,
                 double *prod_deriv_theta_lam_ptr,
                 vector<double> &X_theta_lam_xi_pos,
                 vector<double> &prod_PX_Pxmax_xi_pos,
                 vector<double> &cutoff_xi_pos, XPtr<BigMatrix> xMat, double *y,
                 vector<double> &z, int xmax_idx, int *row_idx,
                 vector<int> &col_idx, NumericVector &center,
                 NumericVector &scale, IntegerVector &ylab, int n_pos, int n,
                 int p) {
  ...
  *g_theta_lam_ptr = dual_bin(theta_lam, 1.0, 1.0, n);
  ...
  double sum_xmaxTy = crossprod_bm(xMat, y, row_idx, center[xmax_idx],
  ...
  for (j = 0; j < p; j++) {
    ...
    prod_PX_Pxmax_xi_pos[j] =
        -sign_xmaxTy * crossprod_bm_Xj_Xk(xMat, row_idx, center, scale, n,
                                          col_idx[j], xmax_idx);
...
void slores_screen(int *slores_reject, vector<double> &theta_lam,
                   ...
                   vector<double> &X_theta_lam_xi_pos,
                   vector<double> &prod_PX_Pxmax_xi_pos,
                   vector<double> &cutoff_xi_pos, int *row_idx,
                   vector<int> &col_idx, NumericVector &center,
                   NumericVector &scale, int xmax_idx, IntegerVector &ylab,
                   double lambda, double lambda_max, int n_pos, int n, int p) {
  ...
  if (fabs(lambda - lambda_max) <= TOLERANCE) {
    ...
    r = sqrt(0.5 * n *
             (dual_bin(theta_lam, lambda, lambda_max, n) - g_theta_lam +
  ...
  int j;
#pragma omp parallel for private(j, a1_xi_pos, a0, Delta, u2star_xi_pos,       \
                                     u2star_xi_neg, T_temp_pos, T_temp_neg,    \
                                     tmp_pos, tmp_neg) schedule(static)
  for (j = 0; j < p; j++) {
    // a1_xi_pos = 0.0;
    // a0 = 0.0; Delta = 0.0;
    ...
    if (Delta < 0.0)
      Delta = 0.0; // in case of -0.0 (at xmax)

    if (cutoff_xi_pos[j] >= d) {
      T_xi_pos[j] = r * sqrt(n) - X_theta_lam_xi_pos[j];
    } else {
      u2star_xi_pos = 0.5 * (-a1_xi_pos + sqrt(Delta)) / a2;
      tmp_pos = n + n * pow(u2star_xi_pos, 2) +
                2 * u2star_xi_pos * prod_PX_Pxmax_xi_pos[j];
      if (tmp_pos < 0.0)
        tmp_pos = 0.0; // in case of -0.0 (at xmax)
      T_temp_pos = sqrt(tmp_pos);
      T_xi_pos[j] = r * T_temp_pos - u2star_xi_pos * n * (lambda_max - lambda) -
                    X_theta_lam_xi_pos[j];
    }

    if (T_xi_pos[j] + TOLERANCE >
        n * lambda) { // cannot reject since T_xi_pos >= n * lambda, no need to
                      // compute T_xi_neg
      slores_reject[j] = 0;
    } else {
      ...
      if (-cutoff_xi_pos[j] >= d) {
        ...
        tmp_neg = n + n * pow(u2star_xi_neg, 2) +
                  2 * u2star_xi_neg * prod_PX_Pxmax_xi_pos[j];
        if (tmp_neg < 0.0)
          tmp_neg = 0.0; // in case of -0.0 (at xmax)
        T_temp_neg = sqrt(tmp_neg);
        T_xi_neg[j] = r * T_temp_neg -
                      u2star_xi_neg * n * (lambda_max - lambda) +
                      X_theta_lam_xi_pos[j];
      }
      if (T_xi_neg[j] + TOLERANCE >
          n * lambda) { // cannot reject since T_xi_neg > n * lambda
        slores_reject[j] = 0;
      } else {
        slores_reject[j] =
            1; // both T_xi_pos and T_xi_pos are less than n * lambda
      }
    }
  }
}
...
void slores_update(vector<double> &theta_lam, vector<double> &z,
                   double sumResid, double *r, double *g_theta_lam_ptr,
                   double *prod_deriv_theta_lam_ptr,
                   ...
                   NumericVector &scale, IntegerVector &ylab, int n, int p) {
...
void slores_update_xmax(vector<double> &prod_PX_Pxmax_xi_pos,
                        vector<double> &cutoff_xi_pos, XPtr<BigMatrix> xMat,
                        double *y, int xmax_idx, int *row_idx,
                        vector<int> &col_idx, NumericVector &center,
...
static bool cdfit_binomial_lambda_core(
    arma::sp_mat &beta, NumericVector &beta0, double *a, double &a0, double *r,
    double *eta, double *s, double *w, double *y, XPtr<BigMatrix> xMat,
    int *row_idx, vector<int> &col_idx, NumericVector &center,
    NumericVector &scale, double *m, double alpha, double lambda_l,
    double thresh, double nullDev, int warn, int max_iter, int n, int p, int l,
    IntegerVector &iter, NumericVector &Dev, vector<double> &z, int *e1,
    int *e2, double &sumS, double *beta_max_ptr, int *beta_max_idx_ptr,
  ...
  while (iter[l] < max_iter) {
    while (iter[l] < max_iter) {
      ...
      if (Dev[l] / nullDev < .01) {
        ...
        for (int ll = l; ll < iter.length(); ll++)
          iter[ll] = NA_INTEGER;
        return true;
      }
      ...
      sumWResid = wsum(
          r, w, n); // update temp result: sum of w * r, used for computing xwr;
      ...
      for (j = 0; j < p; j++) {
        if (e1[j]) {
          jj = col_idx[j];
          xwr = wcrossprod_resid(xMat, r, sumWResid, row_idx, center[jj],
                                 scale[jj], w, n, jj);
          ...
          if (shift != 0) {
            ...
            // update = - u * shift + (0.5 * v + 0.5 * l2) * (pow(beta(j, l), 2)
            // - pow(a[j], 2)) + l1 * (fabs(beta(j, l)) - fabs(a[j]));
            update = pow(beta(j, l) - a[j], 2) * v;
            if (update > max_update)
              max_update = update;
            update_resid_eta(r, eta, xMat, shift, row_idx, center[jj],
                             scale[jj], n, jj); // update r
            sumWResid = wsum(
                r, w, n); // update temp result w * r, used for computing xwr;
    ...
    sumS = sum(s, n);
    violations = check_strong_set(e1, e2, z, xMat, row_idx, col_idx, center,
                                  scale, a, lambda_l, sumS, alpha, s, m, n, p);
    if (violations == 0)
      break;
  }
  return false;
...
RcppExport SEXP cdfit_binomial_ssr(SEXP X_, SEXP y_, SEXP row_idx_,
                                   ...
                                   SEXP lambda_min_, SEXP alpha_, SEXP user_,
  ...
  for (l = lstart; l < L; l++) {
    ...
    while (iter[l] < max_iter) {
      bool saturated = cdfit_binomial_lambda_core(
          beta, beta0, a, a0, r, eta, s, w, y, xMat, row_idx, col_idx, center,
          scale, m, alpha, lambda[l], thresh, nullDev, warn, max_iter, n, p, l,
          iter, Dev, z, e1, e2, sumS, nullptr, nullptr, xmax_idx, nullptr);
      if (saturated) {
        R_Free(s);
        R_Free(w);
        R_Free(a);
...
RcppExport SEXP cdfit_binomial_ssr_approx(
    SEXP X_, SEXP y_, SEXP row_idx_, SEXP lambda_, SEXP nlambda_,
    SEXP lambda_min_, SEXP alpha_, SEXP user_, SEXP eps_, SEXP max_iter_,
    SEXP multiplier_, SEXP dfmax_, SEXP ncore_, SEXP warn_, SEXP verbose_) {
  XPtr<BigMatrix> xMat(X_);
  double *y = REAL(y_);
  int *row_idx = INTEGER(row_idx_);
  double lambda_min = REAL(lambda_min_)[0];
    ...
    while (iter[l] < max_iter) {
      while (iter[l] < max_iter) {
        while (iter[l] < max_iter) {
          ...
          sumResid = sum(
              r, n); // update temp result: sum of r, used for computing xwr;
          ...
          for (j = 0; j < p; j++) {
            if (e1[j]) {
              jj = col_idx[j];
              xwr = 0.25 * crossprod_resid(xMat, r, sumResid, row_idx,
                                           center[jj], scale[jj], n, jj);
              v = 0.25; // x^T * W * x / n = w = 0.25
              // xwr = wcrossprod_resid(xMat, r, sumWResid, row_idx, center[j],
              // scale[j], w, n, j); v = wsqsum_bm(xMat, w, row_idx, center[j],
              // scale[j], n, j) / n;
              u = xwr / n + v * a[j];
              ...
              if (shift != 0) {
                // update change of objective function
                // update = - u * shift + (0.5 * v + 0.5 * l2) * (pow(beta(j,
                // l), 2) - pow(a[j], 2)) + l1 * (fabs(beta(j, l)) -
                // fabs(a[j]));
                update = pow(beta(j, l) - a[j], 2) * v;
                if (update > max_update)
                  max_update = update;
                update_resid_eta(r, eta, xMat, shift, row_idx, center[jj],
                                 scale[jj], n, jj); // update r
                sumResid = sum(
                    r, n); // update temp result w * r, used for computing xwr;
                a[j] = beta(j, l); // update a
              }
            }
          }
        ...
        sumS = sum(s, n);
        violations =
            check_strong_set(e1, e2, z, xMat, row_idx, col_idx, center, scale,
                             a, lambda[l], sumS, alpha, s, m, n, p);
        if (violations == 0)
          break;
      }
...
RcppExport SEXP cdfit_binomial_slores_ssr(
    SEXP X_, SEXP y_, SEXP n_pos_, SEXP ylab_, SEXP row_idx_, SEXP lambda_,
    SEXP nlambda_, SEXP lam_scale_, SEXP lambda_min_, SEXP alpha_, SEXP user_,
    SEXP eps_, SEXP max_iter_, SEXP multiplier_, SEXP dfmax_, SEXP ncore_,
    SEXP warn_, SEXP safe_thresh_, SEXP verbose_) {
  // ProfilerStart("Slores.out");
  XPtr<BigMatrix> xMat(X_);
  double *y = REAL(y_);
  int n_pos = INTEGER(n_pos_)[0];
  IntegerVector ylabel =
      Rcpp::as<IntegerVector>(ylab_); // label vector of {-1, 1}
  int *row_idx = INTEGER(row_idx_);
  double lambda_min = REAL(lambda_min_)[0];
  double alpha = REAL(alpha_)[0];
  int n = Rf_length(row_idx_); // number of observations used for fitting model
  ...
  if (slores_thresh < 1) {
    ...
    slores_init(theta_lam, g_theta_lam_ptr, prod_deriv_theta_lam_ptr,
                cutoff_xi_pos, X_theta_lam_xi_pos, prod_PX_Pxmax_xi_pos, xMat,
                y, z, xmax_idx, row_idx, col_idx, center, scale, ylabel, n_pos,
                n, p);
  } else {
    slores = 0;
  }
  ...
  for (l = lstart; l < L; l++) {
    ...
    if (l != 0) {
      ...
      if (nv > dfmax) {
        for (int ll = l; ll < L; ll++)
          iter[ll] = NA_INTEGER;
        R_Free(slores_reject);
        R_Free(slores_reject_old);
        R_Free(s);
        R_Free(w);
        R_Free(a);
        R_Free(r);
        ...
        // ProfilerStop();
        return List::create(beta0, beta, center, scale, lambda, Dev, iter,
                            n_reject, n_slores_reject, Rcpp::wrap(col_idx));
      }
      cutoff = 2 * lambda[l] - lambda[l - 1];
    } else {
      cutoff = 2 * lambda[l] - lambda_max;
    ...
    if (slores) {
      ...
      // update z[j] for features which are rejected at previous lambda but
      // accepted at current one.
...
#pragma omp parallel for private(j) schedule(static)
      for (j = 0; j < p; j++) {
        ...
        // hsr screening
        // if (slores_reject[j] == 0 && (fabs(z[j]) > (cutoff * alpha *
        // m[col_idx[j]]))) {
        if (fabs(z[j]) > (cutoff * alpha * m[col_idx[j]])) {
          e2[j] = 1;
        } else {
...
#pragma omp parallel for private(j) schedule(static)
    ...
    while (iter[l] < max_iter) {
      ...
      // Scan for violations in rest
      if (slores) {
        violations = check_rest_safe_set(e1, e2, slores_reject, z, xMat,
                                         row_idx, col_idx, center, scale, a,
                                         lambda[l], sumS, alpha, s, m, n, p);
      } else {
        violations =
            check_rest_set(e1, e2, z, xMat, row_idx, col_idx, center, scale, a,
                           lambda[l], sumS, alpha, s, m, n, p);
      }
      if (violations == 0)
        break;
      if (n_slores_reject[l] <= p * slores_thresh) {
        slores =
            0; // turn off slores screening for next iteration if not efficient
      }
    }
  }
  R_Free(slores_reject);
  ...
  // ProfilerStop();
  return List::create(beta0, beta, center, scale, lambda, Dev, iter, n_reject,
                      n_slores_reject, Rcpp::wrap(col_idx));
}

// Coordinate descent for logistic models with ada-slores_ssr
RcppExport SEXP cdfit_binomial_ada_slores_ssr(
    SEXP X_, SEXP y_, SEXP n_pos_, SEXP ylab_, SEXP row_idx_, SEXP lambda_,
    SEXP nlambda_, SEXP lam_scale_, SEXP lambda_min_, SEXP alpha_, SEXP user_,
    SEXP eps_, SEXP max_iter_, SEXP multiplier_, SEXP dfmax_, SEXP ncore_,
  ...
  for (l = lstart; l < L; l++) {
    ...
    if (slores) {
      ...
      if (gain - n_slores_reject[l - 1] * (l - l_prev) >
              update_thresh * (xmax_invalid + 1) * p &&
          l != L - 1) {
        l_prev = l - 1;
        ...
        if (xmax_invalid) {
          ...
          xmax_invalid = 0;
          slores_update_xmax(prod_PX_Pxmax_xi_pos, cutoff_xi_pos, xMat, y,
                             xmax_idx, row_idx, col_idx, center, scale, n, p);
        }
        slores_update(theta_lam, z, sumS, s, g_theta_lam_ptr,
                      prod_deriv_theta_lam_ptr, X_theta_lam_xi_pos,
                      lambda[l_prev], xMat, eta, xmax_idx, row_idx, col_idx,
                      center, scale, ylabel, n, p);

        slores_screen(slores_reject, theta_lam, g_theta_lam,
                      prod_deriv_theta_lam, X_theta_lam_xi_pos,
                      prod_PX_Pxmax_xi_pos, cutoff_xi_pos, row_idx, col_idx,
                      center, scale, xmax_idx, ylabel, lambda[l],
                      lambda[l_prev], n_pos, n, p);
        n_slores_reject[l] = sum(slores_reject, p);
        ...
        // update z[j] for features which are rejected at previous lambda but
        // accepted at current one.
        // update_zj(z, slores_reject, slores_reject_old, xMat, row_idx,
        // col_idx, center, scale, sumS, s, m, n, p);
      }
...
#pragma omp parallel for private(j) schedule(static)
    ...
    while (iter[l] < max_iter) {
      bool saturated = cdfit_binomial_lambda_core(
          beta, beta0, a, a0, r, eta, s, w, y, xMat, row_idx, col_idx, center,
          scale, m, alpha, lambda[l], thresh, nullDev, warn, max_iter, n, p, l,
          iter, Dev, z, e1, e2, sumS, &beta_max, &beta_max_idx, xmax_idx,
          &xmax_invalid);
      if (saturated) {
        R_Free(slores_reject);
        R_Free(slores_reject_old);
