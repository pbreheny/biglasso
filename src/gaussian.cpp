...
// apply EDPP
void edpp_screen(int *discard_beta, int n, int p, double rhs2, double *Xtr,
                 double *lhs2, double c, double c1, double *m, double alpha,
                 vector<int> &col_idx) {
  int j;
  for (j = 0; j < p; j++) {
    if (fabs(c1 * Xtr[j] + c / 2 * lhs2[j]) <
        n * alpha * m[col_idx[j]] - c / 2 * rhs2) {
      discard_beta[j] = 1;
    } else {
      discard_beta[j] = 0;
    }
...
void edpp_update(XPtr<BigMatrix> xpMat, double *r, double sumResid,
                 double *lhs2, double *Xty, double *Xtr, double *yhat,
                 double ytyhat, double yhat_norm2, int *row_idx,
...
void bedpp_init(vector<double> &sign_lammax_xtxmax, XPtr<BigMatrix> xMat,
                int xmax_idx, double *y, double lambda_max, int *row_idx,
                vector<int> &col_idx, NumericVector &center,
                NumericVector &scale, int n, int p) {
  ...
  int j, jj;
  // sign of xmaxTy
  sum_xmaxTy = crossprod_bm(xMat, y, row_idx, center[xmax_idx], scale[xmax_idx],
                            n, xmax_idx);
...
#pragma omp parallel for private(j, sum_xjxmax) schedule(static)
  for (j = 0; j < p; j++) { // p = p_keep
    ...
    if (jj != xmax_idx) {
      ...
      sign_lammax_xtxmax[j] = sign_xmaxTy * lambda_max *
                              (sum_xjxmax - n * center[jj] * center[xmax_idx]) /
                              (scale[jj] * scale[xmax_idx]);
      ;
    } else {
...
void bedpp_screen(int *bedpp_reject, const vector<double> &sign_lammax_xtxmax,
  ...
  double LHS = 0.0;
  double RHS =
      2 * n * alpha * lambda * lambda_max -
      (lambda_max - lambda) * sqrt(n * ynorm_sq * (1 + lambda * (1 - alpha)) -
                                   pow(n * alpha * lambda_max, 2));
  int j;

#pragma omp parallel for private(j, LHS) schedule(static)
  for (j = 0; j < p; j++) { // p = p_keep
    LHS = (lambda + lambda_max) * XTy[j] - (lambda_max - lambda) * alpha *
                                               sign_lammax_xtxmax[j] /
                                               (1 + lambda * (1 - alpha));
    if (fabs(LHS) < RHS) {
      bedpp_reject[j] = 1;
    } else {
...
// shared inner CD loop behind cdfit_gaussian_ssr(),
// cdfit_gaussian_ada_edpp_ssr(), and cdfit_gaussian_bedpp_ssr(): sweeps the
// ever-active set to convergence, then checks the strong set for violations
// and, if none, the rest set -- gated by a safe-rule discard set
// (`discard_beta`, e.g. EDPP/BEDPP rejections) when one is supplied, or ungated
// (plain check_rest_set()) when `discard_beta` is nullptr -- repeating until
// lambda[l] converges with no violations anywhere. Mutates `beta`, `a`, `r`,
// `sumResid`, `z`, `ever_active`, `strong_set`, `iter[l]`, and `loss[l]` (the
// last only set on convergence) in place.
static void cdfit_gaussian_lambda_core(
    arma::sp_mat &beta, double *a, double *r, double &sumResid,
    vector<double> &z, int *ever_active, int *strong_set, int *discard_beta,
    XPtr<BigMatrix> xMat, int *row_idx, vector<int> &col_idx,
    NumericVector &center, NumericVector &scale, double *m, double alpha,
    double lambda_l, double thresh, int max_iter, int n, int p, int l,
    IntegerVector &iter, NumericVector &loss) {
  ...
  while (iter[l] < max_iter) {
    while (iter[l] < max_iter) {
      while (iter[l] < max_iter) {
        ...
        for (j = 0; j < p; j++) {
          if (ever_active[j]) {
            jj = col_idx[j];
            z[j] = crossprod_resid(xMat, r, sumResid, row_idx, center[jj],
                                   scale[jj], n, jj) /
                       n +
                   a[j];
            ...
            if (shift != 0) {
              ...
              update_resid(xMat, r, shift, row_idx, center[jj], scale[jj], n,
                           jj);     // update r
      ...
      violations = check_strong_set(ever_active, strong_set, z, xMat, row_idx,
                                    col_idx, center, scale, a, lambda_l,
    ...
    if (discard_beta == nullptr) {
      violations = check_rest_set(ever_active, strong_set, z, xMat, row_idx,
                                  col_idx, center, scale, a, lambda_l, sumResid,
                                  alpha, r, m, n, p);
    } else {
      violations = check_rest_safe_set(ever_active, strong_set, discard_beta, z,
                                       xMat, row_idx, col_idx, center, scale, a,
                                       lambda_l, sumResid, alpha, r, m, n, p);
    }
    if (violations == 0) {
      loss[l] = gLoss(r, n);
      break;
...
RcppExport SEXP cdfit_gaussian_ssr(SEXP X_, SEXP y_, SEXP row_idx_,
  ...
  for (l = lstart; l < L; l++) {
    ...
    cdfit_gaussian_lambda_core(beta, a, r, sumResid, z, e1, e2, nullptr, xMat,
                               row_idx, col_idx, center, scale, m, alpha,
                               lambda[l], thresh, max_iter, n, p, l, iter,
                               loss);
  }
...
RcppExport SEXP cdfit_gaussian_ada_edpp_ssr(
    SEXP X_, SEXP y_, SEXP row_idx_, SEXP lambda_, SEXP nlambda_,
    SEXP lam_scale_, SEXP lambda_min_, SEXP alpha_, SEXP user_, SEXP eps_,
  ...
  double xty = sign(crossprod_bm(xMat, y, row_idx, center[xmax_idx],
  ...
  for (l = lstart; l < L; l++) {
    ...
    if (l != lstart) {
      ...
      if (gain - n_safe_reject[l - 1] * (l - l_prev) > update_thresh * p &&
        ...
        edpp_update(xMat, r, sumResid, lhs2, Xty, Xtr, yhat, ytyhat, yhat_norm2,
        ...
        // Reapply EDPP
        edpp_screen(discard_beta, n, p, rhs2, Xtr, lhs2, c, 1 / lambda[l_prev],
                    m, alpha, col_idx);
      ...
      for (j = 0; j < p; j++) {
        jj = col_idx[j];
        xjtx =
            crossprod_bm_Xj_Xk(xMat, row_idx, center, scale, n, jj, xmax_idx);
      ...
      c = (lambda_max - lambda[l]) / lambda_max / lambda[l];
      edpp_screen(discard_beta, n, p, rhs2, Xtr, lhs2, c, 1 / lambda_max, m,
                  alpha, col_idx);
      n_safe_reject[l] = sum(discard_beta, p);
      gain = n_safe_reject[l];
    }
    ...
    // strong set
    // update_zj(z, discard_beta, discard_old, xMat, row_idx, col_idx, center,
    // scale, sumResid, r, m, n, p);
    if (l != lstart)
      cutoff = 2 * lambda[l] - lambda[l - 1];
    for (j = 0; j < p; j++) {
      if (discard_beta[j])
    ...
    cdfit_gaussian_lambda_core(beta, a, r, sumResid, z, ever_active, strong_set,
                               discard_beta, xMat, row_idx, col_idx, center,
                               scale, m, alpha, lambda[l], thresh, max_iter, n,
                               p, l, iter, loss);
  }
...
RcppExport SEXP cdfit_gaussian_bedpp_ssr(SEXP X_, SEXP y_, SEXP row_idx_,
  ...
  if (bedpp_thresh < 1) {
    ...
    bedpp_init(sign_lammax_xtxmax, xMat, xmax_idx, y, lambda_max, row_idx,
               col_idx, center, scale, n, p);
  } else {
  ...
  for (l = lstart; l < L; l++) {
    ...
    if (l != 0) {
      ...
      if (nv > dfmax) {
        ...
        R_Free(e2);
        R_Free(bedpp_reject);
        R_Free(bedpp_reject_old);
        return List::create(beta, center, scale, lambda, loss, iter, n_reject,
                            n_bedpp_reject, Rcpp::wrap(col_idx));
      }
      cutoff = 2 * lambda[l] - lambda[l - 1];
    } else {
      cutoff = 2 * lambda[l] - lambda_max;
...
#pragma omp parallel for private(j) schedule(static)
      for (j = 0; j < p; j++) {
        ...
        // hsr screening
        if (bedpp_reject[j] == 0 &&
            (fabs(z[j]) >= (cutoff * alpha * m[col_idx[j]]))) {
          e2[j] = 1;
        } else {
          e2[j] = 0;
        }
...
#pragma omp parallel for private(j) schedule(static)
    ...
    cdfit_gaussian_lambda_core(beta, a, r, sumResid, z, e1, e2,
                               bedpp ? bedpp_reject : nullptr, xMat, row_idx,
                               col_idx, center, scale, m, alpha, lambda[l],
                               thresh, max_iter, n, p, l, iter, loss);
