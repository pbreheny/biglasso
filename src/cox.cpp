...
// standardize
void standardize_and_get_residual_cox(
    NumericVector &center, NumericVector &scale, int *p_keep_ptr,
    vector<int>
        &col_idx, // columns to keep, removing columns whose scale < 1e-6
    vector<double> &z, double *lambda_max_ptr, int *xmax_ptr,
    XPtr<BigMatrix> xMat, double *y, double *d, int *d_idx, int *row_idx,
    double alpha, int n, int f, int p, double *m) {
  ...
  for (j = 0; j < p; j++) {
    ...
    if (scale[j] > 1e-6) {
      ...
      if (m[j] > 0 && fabs(zj) / m[j] > zmax) {
        zmax = fabs(zj) / m[j];
        *xmax_ptr =
            j; // xmax_ptr is the index in the raw xMat, not index in col_idx!
      }
      z.push_back(zj);
    }
  }
...
// SAFE initialization
void safe_init(vector<double> &scale_SAFE_X, XPtr<BigMatrix> xMat, double *haz,
               double *rsk, vector<double> &z, int xmax_col_idx, int *row_idx,
...
#pragma omp parallel for private(j, jj, i, k, max_x, min_x, sum_min, sum_max,  \
                                     xTy) schedule(static)
  for (j = 0; j < p; j++) {
    jj = col_idx[j];
    xCol = xAcc[jj];
    ...
    for (k = f - 1; k >= 0; k--) {
      for (; i >= 0 && d_idx[i] >= k; i--) {
        if (xCol[row_idx[i]] > max_x)
          max_x = xCol[row_idx[i]];
        if (xCol[row_idx[i]] < min_x)
          min_x = xCol[row_idx[i]];
        if (y[i] > 0)
          xTy += xCol[row_idx[i]];
      }
      sum_max += d[k] * max_x;
    ...
    sum_min = (xTy - sum_min);
    scale_SAFE_X[j] =
        (sum_max > sum_min) ? sum_max / scale[jj] / n : sum_min / scale[jj] / n;
    Rprintf("SAFE:%f\n", scale_SAFE_X[j]);
  }
}

// SAFE screening
void safe_screen(int *safe_reject, double lambda, int p,
...
double dual_cox(double *haz, double *rsk, double lambda, double lambda_0, int n,
                int f, double *y, double *d, int *d_idx) {
  double res = 0.0;
  ...
  for (i = 0; i < n; i++) {
    if (y[i] == 1) {
      k = d_idx[i];
      res +=
          ((1 - lam_ratio) + lam_ratio * d[k] * haz[i] / rsk[k]) *
...
// Scox initialization
void scox_init(double *g_theta_lam_ptr, vector<double> &scaleP_X,
               vector<double> &X_theta_lam, XPtr<BigMatrix> xMat, double *haz,
               double *rsk, vector<double> &z, int *row_idx,
               vector<int> &col_idx, NumericVector &center,
               NumericVector &scale, int n, int p, int f, double *y, double *d,
               int *d_idx) {
  ...
  *g_theta_lam_ptr = dual_cox(haz, rsk, 1.0, 1.0, n, f, y, d, d_idx);
  ...
  for (i = 0; i < n; i++) {
    ...
    k = d_idx[i];
    prod_deriv_theta_lam += (d[k] * haz[i] / rsk[k] - y[i]) * log(haz[i] /
  rsk[k]);
  }
...
void scox_update(vector<double> &X_theta_lam, vector<double> &z, double *eta,
                 double *haz0, double *rsk0, XPtr<BigMatrix> xMat, int *row_idx,
                 vector<int> &col_idx, NumericVector &center,
                 NumericVector &scale, int n, int p, int f, double *y,
                 double *d, int *d_idx) {
...
void scox_updater(double *g_theta_lam_ptr, double *eta, double lambda, double l,
                  ...
                  int f, double *y, double *d, int *d_idx, int max_iter,
                  double thresh, int *e1, double *m, double alpha, double *a,
                  arma::sp_mat &beta) {
    ...
    for (j = 0; j < p; j++) {
      if (e1[j]) {
        ...
        if (shift != 0) {
          ...
          if (fabs(beta(j, l)) == 10)
            max_update = 10;
          update_resid_eta(r, eta, xMat, shift, row_idx, center[jj], scale[jj],
                           n, jj); // update r
          sumWResid = wsum(
...
double prod_deriv_theta(double *haz0, double *rsk0, double *haz, double *rsk,
  ...
  for (i = 0; i < n; i++) {
    ...
    k = d_idx[i];
    prod_deriv_theta_lam +=
        (d[k] * haz[i] / rsk[k] - y[i]) * log(haz0[i] / rsk0[k]);
  }
...
double primal(double *beta, double lambda, double lambda0, int n, int p, int f,
              double *y, double *d, int *d_idx) {
  double res = 0;
  for (int j = 0; j < p; j++)
    res += fabs(beta[j]);
...
// Scox screening
void scox_screen(int *scox_reject, double lambda, double lambda_0, double *haz,
                 double *rsk, double g_theta_lam, vector<double> &scaleP_X,
                 vector<double> &X_theta_lam, int *row_idx,
                 vector<int> &col_idx, NumericVector &center,
                 NumericVector &scale, int n, int p, int f, double *y,
                 double *d, int *d_idx, int *e1) {
...
#pragma omp parallel for private(j, T) schedule(static)
  for (j = 0; j < p; j++) {
    ...
    // X_theta_lam[j]*lam_ratio, r/sqrt(n), scaleP_X[j]/sqrt(n), lambda);
    ...
    T = -lam_ratio * X_theta_lam[j] + r * scaleP_X[j] / n;
    // if(j==p-1) Rprintf("T-[%i]/lam=%f+%f*%f/%f\n", j,
    // -lam_ratio*X_theta_lam[j], r/sqrt(n), scaleP_X[j]/sqrt(n), lambda);
...
// value.
static bool compute_cox_deviance(double *haz, double *rsk, double *eta,
                                 int *d_idx, double *y, NumericVector &Dev,
                                 double satDev, double nullDev, int n, int f,
                                 int l) {
  int i, k;
  Dev[l] = 0.0;
...
RcppExport SEXP cdfit_cox(SEXP X_, SEXP y_, SEXP d_, SEXP d_idx_, SEXP row_idx_,
  ...
  int *d_idx = INTEGER(
      d_idx_); // Index of unique failure time for subjects with failure; Index
               // of the last unique failure time if censored
  ...
  // xmax_idx;
  standardize_and_get_residual_cox(center, scale, p_keep_ptr, col_idx, z,
                                   lambda_max_ptr, xmax_ptr, xMat, y, d, d_idx,
                                   row_idx, alpha, n, f, p, m);
  ...
  for (l = lstart; l < L; l++) {
    ...
    if (l != 0) {
      ...
      if (nv > dfmax) {
        ...
        R_Free(haz);
        R_Free(rsk);
        return List::create(beta, center, scale, lambda, Dev, iter, n_reject,
                            Rcpp::wrap(col_idx));
      }
    }
    ...
    while (iter[l] < max_iter) {
      while (iter[l] < max_iter) {
        ...
        if (compute_cox_deviance(haz, rsk, eta, d_idx, y, Dev, satDev, nullDev,
                                 n, f, l)) {
          if (warn)
            warning("Model saturated with deviance %f; exiting...", Dev[l]);
          for (int ll = l; ll < L; ll++)
            iter[ll] = NA_INTEGER;
          R_Free(s);
          R_Free(w);
      ...
      violations =
          check_inactive_set(e1, z, xMat, row_idx, col_idx, center, scale, a,
                             lambda[l], sumWResid, alpha, s, m, n, p);
      if (violations == 0)
        break;
    }
  }
...
RcppExport SEXP cdfit_cox_ssr(SEXP X_, SEXP y_, SEXP d_, SEXP d_idx_,
    ...
    while (iter[l] < max_iter) {
      while (iter[l] < max_iter) {
        while (iter[l] < max_iter) {
          ...
          if (compute_cox_deviance(haz, rsk, eta, d_idx, y, Dev, satDev,
                                   nullDev, n, f, l)) {
            if (warn)
              warning("Model saturated with deviance %f; exiting...", Dev[l]);
            for (int ll = l; ll < L; ll++)
          ...
          for (j = 0; j < p; j++) {
            if (e1[j]) {
              ...
              if (shift != 0) {
                ...
                sumWResid = wsum(
                    r, w,
                    n); // update temp result w * r, used for computing xwr;
                a[j] = beta(j, l); // update a
              }
            }
          }
        ...
        violations =
            check_strong_set(e1, e2, z, xMat, row_idx, col_idx, center, scale,
                             a, lambda[l], sumWResid, alpha, s, m, n, p);
        if (violations == 0)
          break;
      }
...
RcppExport SEXP cdfit_cox_scox(SEXP X_, SEXP y_, SEXP d_, SEXP d_idx_,
                               SEXP row_idx_, SEXP lambda_, SEXP nlambda_,
                               SEXP lam_scale_, SEXP lambda_min_, SEXP alpha_,
                               SEXP user_, SEXP eps_, SEXP max_iter_,
                               SEXP multiplier_, SEXP dfmax_, SEXP ncore_,
  ...
  for (l = lstart; l < L; l++) {
    ...
    if (scox) {
      scox_screen(safe_reject, lambda[l], lambda_max, haz0, rsk0, g_theta_lam,
                  scaleP_X, X_theta_lam, row_idx, col_idx, center, scale, n, p,
                  f, y, d, d_idx, e1);
    }
    ...
    while (iter[l] < max_iter) {
      ...
      violations =
          check_safe_set(e1, safe_reject, z, xMat, row_idx, col_idx, center,
                         scale, a, lambda[l], sumWResid, alpha, s, m, n, p);
      if (violations == 0)
        break;
    }
  }
...
RcppExport SEXP cdfit_cox_sscox(SEXP X_, SEXP y_, SEXP d_, SEXP d_idx_,
                                SEXP row_idx_, SEXP lambda_, SEXP nlambda_,
                                SEXP lam_scale_, SEXP lambda_min_, SEXP alpha_,
                                SEXP user_, SEXP eps_, SEXP max_iter_,
                                SEXP multiplier_, SEXP dfmax_, SEXP ncore_,
  ...
  for (l = lstart; l < L; l++) {
    ...
    if (scox) {
      ...
      scox_screen(safe_reject, lambda[l], lambda[l - 1], haz0, rsk0,
                  g_theta_lam, scaleP_X, X_theta_lam, row_idx, col_idx, center,
                  scale, n, p, f, y, d, d_idx, e1);
    }
    n_reject[l] = sum(safe_reject, p);
    if (n_reject[l] == 0)
      scox = 0;
...
RcppExport SEXP cdfit_cox_ada_scox(
    SEXP X_, SEXP y_, SEXP d_, SEXP d_idx_, SEXP row_idx_, SEXP lambda_,
  ...
  for (l = lstart; l < L; l++) {
    ...
    if (l != 0) {
      ...
      if (nv > dfmax) {
        ...
        R_Free(rsk);
        return List::create(beta, center, scale, lambda, Dev, iter, n_reject,
                            n_safe_reject, Rcpp::wrap(col_idx));
      }
    }

    if (scox) {
      if (gain - n_safe_reject[l - 1] * (l - l_prev) > update_thresh * p &&
          l != L - 1) {
        l_prev = l - 1;
        scox_update(X_theta_lam, z, eta, haz0, rsk0, xMat, row_idx, col_idx,
                    center, scale, n, p, f, y, d, d_idx);
        scox_updater(g_theta_lam_ptr, eta, lambda[l], l, xMat, row_idx, col_idx,
                     ...
                     m, alpha, a, beta);
        scox_screen(safe_reject, lambda[l], lambda[l - 1], haz0, rsk0,
                    g_theta_lam, scaleP_X, X_theta_lam, row_idx, col_idx,
                    center, scale, n, p, f, y, d, d_idx, e1);
        n_safe_reject[l] = sum(safe_reject, p);
        gain = n_safe_reject[l];
        if (n_safe_reject[l] <= safe_thresh * p)
          scox = 0;
      } else {
        scox_updater(g_theta_lam_ptr, eta, lambda[l], l, xMat, row_idx, col_idx,
                     center, scale, n, p, f, y, d, d_idx, max_iter, thresh, e1,
                     m, alpha, a, beta);
        scox_screen(safe_reject, lambda[l], lambda[l_prev], haz0, rsk0,
                    g_theta_lam, scaleP_X, X_theta_lam, row_idx, col_idx,
                    center, scale, n, p, f, y, d, d_idx, e1);
        n_safe_reject[l] = sum(safe_reject, p);
        gain += n_safe_reject[l];
    ...
    while (iter[l] < max_iter) {
      ...
      violations = check_rest_safe_set(e1, e2, safe_reject, z, xMat, row_idx,
                                       col_idx, center, scale, a, lambda[l],
                                       sumWResid, alpha, s, m, n, p);
      if (violations == 0)
        break;
    }
  }
...
RcppExport SEXP cdfit_cox_safe(SEXP X_, SEXP y_, SEXP d_, SEXP d_idx_,
                               SEXP row_idx_, SEXP lambda_, SEXP nlambda_,
                               SEXP lam_scale_, SEXP lambda_min_, SEXP alpha_,
                               SEXP user_, SEXP eps_, SEXP max_iter_,
                               SEXP multiplier_, SEXP dfmax_, SEXP ncore_,
  ...
  if (safe_thresh < 1) {
    ...
    safe_init(scale_SAFE_X, xMat, haz, rsk, z, xmax_col_idx, row_idx, col_idx,
              center, scale, n, p, f, y, d, d_idx);
  } else {
    scox = 0;
  }
  ...
  for (l = lstart; l < L; l++) {
    ...
    while (iter[l] < max_iter) {
      ...
      violations =
          check_safe_set(e1, safe_reject, z, xMat, row_idx, col_idx, center,
                         scale, a, lambda[l], 0.0, alpha, s, m, n, p);
      if (violations == 0)
        break;
    }
  }
