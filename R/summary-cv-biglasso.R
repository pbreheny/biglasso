...
#'
#' @returns `summary.cv.biglasso` produces an object with S3 class `"summary.cv.biglasso"`. The
...
#' }
#'
#' @seealso [biglasso()], [cv.biglasso()], [plot.cv.biglasso()]
#'
...
print.summary.cv.biglasso <- function(x, digits, ...) {
  ...
  cat(
    ...
    formatC(min(x$cve, na.rm = TRUE), digits[1], format = "f"),
    "\n",
    sep = ""
  )
  cat(
    "  R-squared: ",
    formatC(max(x$r.squared, na.rm = TRUE), digits[3], format = "f"),
    "\n",
    sep = ""
  )
  cat(
    "  Signal-to-noise ratio: ",
    formatC(max(x$snr, na.rm = TRUE), digits[4], format = "f"),
    "\n",
    sep = ""
  )
  if (x$model == "logistic") {
