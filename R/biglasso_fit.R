...
#' @param r Residuals (length n vector) corresponding to `init`. WARNING: If you supply an incorrect
#'   value of `r`, the solution will be incorrect.
#' @param init Initial values for beta. Default: zero (length p vector)
#' @param xtx X scales: the jth element should equal `crossprod(X[,j])/n`. In particular, if X is
#'   standardized, one should pass `xtx = rep(1, p)`. WARNING: If you supply an incorrect value of
#'   `xtx`, the solution will be incorrect. (length p vector)
#' @param penalty String specifying which penalty to use. Default is 'lasso', Other options are
#'   'SCAD' and 'MCP' (the latter are non-convex)
...
#' @param gamma Tuning parameter value for nonconvex penalty. Defaults are 3.7 for
