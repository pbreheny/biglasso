...
#'   "lambda.1se" (Largest value of `lambda` for which the cross-validation error was at most one
#'   standard error larger than the minimum.).
#' @param which Indices of the penalty parameter `lambda` at which predictions are requested. The
#'   default value is the index of lambda corresponding to lambda.min. Note: this is overridden if
#'   `lambda` is specified.
#' @param \dots Not used.
#'
