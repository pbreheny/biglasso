...
#'
#' Perform k-fold cross validation for penalized regression models over a grid of values for the
...
#' @param row.idx The integer vector of row indices of `X` that used for fitting the model. as in
#'   `biglasso`.
#' @param family Either `"gaussian"`, `"binomial"`, `"cox"` or `"mgaussian"` depending on the
#'   response. `"cox"` and `"mgaussian"` are not supported yet.
#' @param eval.metric The evaluation metric for the cross-validated error and for choosing optimal
#'   `lambda`. "default" for linear regression is MSE (mean squared error), for logistic regression
#'   is binomial deviance. "MAPE", for linear regression only, is the Mean Absolute Percentage
...
#' @param cv.ind Which fold each observation belongs to. By default the observations are randomly
#'   assigned by `cv.biglasso`.
#' @param trace If set to TRUE, cv.biglasso will inform the user of its progress by announcing the
