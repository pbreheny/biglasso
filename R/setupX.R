...
#' \href{https://CRAN.R-project.org/package=bigmemory}{bigmemory} for more details.
#'
#' @param filename The name of the data file. For example, "dat.txt".
#' @param dir The directory used to store the binary and descriptor files associated with the
#'   `big.matrix`. The default is current working directory.
#' @param sep The field separator character. For example, "," for comma-delimited files (the
#'   default); "\\t" for tab-delimited files.
#' @param backingfile The binary file associated with the file-backed `big.matrix`. By default, its
#'   name is the same as `filename` with the extension replaced by ".bin".
#' @param descriptorfile The descriptor file used for the description of the file-backed
#'   `big.matrix`. By default, its name is the same as `filename` with the extension replaced by
#'   ".desc".
